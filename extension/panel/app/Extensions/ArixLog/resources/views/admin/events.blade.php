...
@section('arixlog-title', 'Registro')
@section('arixlog-heading', 'Registro de la extension')
@section('arixlog-subheading', 'todo lo que ha hecho ArixLog')
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover arixlog-table">
                            ...
                            @forelse($events as $event)
                                <tr class="arixlog-level-{{ $event->level }}">
                                    <td class="arixlog-small">{{ optional($event->created_at)->format('d/m/Y H:i:s') }}</td>
                                    <td><span class="arixlog-chip">{{ $event->channel }}</span></td>
                                    <td>{{ $event->message }}</td>
                                    <td class="arixlog-small arixlog-muted">
                                        @if($event->context)
                            ...
                            @empty
                                <tr><td colspan="4"><p class="arixlog-empty">No hay nada registrado todavia.</p></td></tr>
                            @endforelse
                            </tbody>
                        </table>
                    </div>
