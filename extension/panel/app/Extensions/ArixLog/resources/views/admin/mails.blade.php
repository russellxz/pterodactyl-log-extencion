...
@section('arixlog-title', 'Correos')
@section('arixlog-heading', 'Correos enviados')
@section('arixlog-subheading', 'a que cliente fue cada uno y si llego')

@section('arixlog-content')
    <div class="row">
        <div class="col-xs-12">
            <div class="arixlog-stats">
                @include('arixlog::admin.partials.stat', ['icon' => 'check-circle', 'value' => $summary['sent'], 'label' => 'enviados', 'tone' => 'ok'])
                @include('arixlog::admin.partials.stat', ['icon' => 'x-circle', 'value' => $summary['failed'], 'label' => 'fallidos', 'tone' => $summary['failed'] ? 'danger' : ''])
                @include('arixlog::admin.partials.stat', ['icon' => 'clock', 'value' => $summary['queued'], 'label' => 'sin confirmar', 'tone' => $summary['queued'] ? 'warning' : ''])
            </div>
        </div>
    </div>
    ...
    <div class="row">
                            ...
                            @forelse($mails as $mail)
                                <tr>
                                    <td>
                                        ...
                                        <div class="arixlog-muted arixlog-small">{{ $mail->to_addresses }}</div>
                                    </td>
                                    <td>
                                        {{ $mail->subject ?: '(sin asunto)' }}
                                        @if($mail->resend_count > 0)
                                            <span class="arixlog-chip">reenviado {{ $mail->resend_count }}x</span>
                                        @endif
                                    </td>
                                    <td>
                                        @php
                                        ...
                                        @if($mail->error)
                                            <div class="arixlog-muted arixlog-small">{{ \Illuminate\Support\Str::limit($mail->error, 70) }}</div>
                                        @endif
                                    </td>
                                    <td class="arixlog-small">{{ optional($mail->created_at)->format('d/m/Y H:i') }}</td>
                                    <td class="text-right">
                                        <a href="{{ route('admin.arixlog.mails.show', $mail->id) }}" class="btn btn-xs btn-default">
                                            Ver
                                        </a>
        ...
        <div class="col-md-4">
            <div class="box">
                ...
                <form method="POST" action="{{ route('admin.arixlog.mails.test') }}">
                    {{ csrf_field() }}
                    <div class="box-body">
                        <p class="text-muted">
                    ...
                    <div class="box-footer">
                        <button type="submit" class="btn btn-primary">@arixicon('send', 14) Enviar prueba</button>
                    </div>
                </form>
            </div>
            ...
            <div class="box">
                ...
                <div class="box-body arixlog-small">
                    ...
                    <p class="arixlog-muted" style="margin-top:12px;">
