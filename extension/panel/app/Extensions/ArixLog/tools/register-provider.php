...
/**
 * Registra (o quita) el proveedor de ArixLog en config/app.php.
 *
...
const PROVIDER = 'Pterodactyl\\Extensions\\ArixLog\\ArixLogServiceProvider::class';
const MARKER_START = '// ArixLog START (gestionado por la extension, no editar a mano)';
const MARKER_END = '// ArixLog END';
...
$registered = str_contains($contents, 'ArixLogServiceProvider');
...
if ($mode === 'remove') {
    ...
    foreach ($lines as $line) {
        ...
        if (str_contains($line, 'ArixLogServiceProvider')) {
            continue;
        }
    ...
    echo "Proveedor de ArixLog quitado de config/app.php\n";
    exit(0);
}
...
echo "Proveedor de ArixLog registrado en config/app.php\n";
exit(0);
...
function write(string $path, string $contents, string $panel): void
{
    ...
    if (!is_file($path . '.arixlog-backup')) {
        @copy($path, $path . '.arixlog-backup');
    }
