...
use Pterodactyl\Extensions\ArixLog\Models\UpdateRun;
...
class UpdaterController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('arixlog::admin.updater', [
    ...
    public function check()
    {
        ...
        if ($latest['error']) {
            return back()->with('arixlog_error', 'No se pudo consultar la ultima version: ' . $latest['error']);
        }
    ...
    public function start(Request $request)
    {
        ...
        if (UpdateRun::query()->where('status', UpdateRun::STATUS_RUNNING)->exists()) {
            return back()->with('arixlog_error', 'Ya hay una actualizacion en marcha.');
        }
        ...
        try {
            ...
            return back()->with('arixlog_error', 'No se pudo preparar la actualizacion: ' . $e->getMessage());
        }
        ...
        try {
            ...
            return back()->with('arixlog_error', $e->getMessage());
        }

        return redirect()
            ->route('admin.arixlog.updater')
            ->with('arixlog_success', 'Actualizacion lanzada. El panel entrara en mantenimiento unos minutos.')
            ->with('arixlog_watch', $run->token);
    }
     ...
     * /extensions/arixlog/runs/<token>/status.json, que sirve el servidor web
     * directamente sin pasar por PHP.
     */
    public function status(int $run): JsonResponse
    {
        $model = UpdateRun::query()->findOrFail($run);
        ...
        return new JsonResponse([
            ...
            'static_url' => '/extensions/arixlog/runs/' . $model->token . '/status.json',
        ]);
    }

    public function rollback(Request $request, int $run)
    {
