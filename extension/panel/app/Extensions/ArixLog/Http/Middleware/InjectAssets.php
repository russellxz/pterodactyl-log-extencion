...
class InjectAssets
{
    ...
    public function handle(Request $request, \Closure $next): mixed
    {
        ...
        try {
            ...
            try {
                logger()->debug('[ArixLog] no se pudieron inyectar los recursos: ' . $e->getMessage());
            } catch (\Throwable) {
    ...
    private function inject(Request $request, mixed $response): mixed
    {
        ...
        if (str_contains($content, 'data-arixlog="1"')) {
            return $response;
        }
    ...
    private function adminTags(): string
    {
        ...
        return implode('', [
            '<link data-arixlog="1" rel="stylesheet" href="' . $base . '/admin.css?v=' . $this->version() . '">',
            '<script data-arixlog="1" src="' . $base . '/admin.js?v=' . $this->version() . '" defer></script>',
        ]);
    }
    ...
    private function clientTags(Request $request): string
    {
        ...
        return implode('', [
            '<link data-arixlog="1" rel="stylesheet" href="' . $base . '/client.css?v=' . $this->version() . '">',
            '<script data-arixlog="1">window.ArixLogConfig=' . json_encode($config, JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) . ';</script>',
            '<script data-arixlog="1" src="' . $base . '/client.js?v=' . $this->version() . '" defer></script>',
        ]);
    }
    ...
    {
        return '/extensions/arixlog';
    }

    private function version(): string
    {
        return ArixLogServiceProvider::VERSION;
    }
}
