...
class PanelUpdater
{
    ...
    {
        $key = 'arixlog:latest-panel-version';
    ...
    private function fetchLatest(): array
    {
        ...
        try {
            $response = Http::timeout(10)
                ->withHeaders(['Accept' => 'application/vnd.github+json', 'User-Agent' => 'ArixLog'])
    ...
    public function prepare(?string $targetVersion = null, ?int $userId = null, bool $restoreTheme = true): UpdateRun
    {
        ...
        if ($backupRoot === '' || !str_starts_with($backupRoot, '/') || str_starts_with($backupRoot . '/', $panel . '/')) {
            throw new \RuntimeException(
                ...
                . 'fuera de la carpeta del panel. Cambiala en la configuracion de ArixLog.'
            );
        }
    ...
    {
        return public_path('extensions/arixlog/runs/' . $token);
    }
    ...
    private function writeScript(UpdateRun $run, string $version, ?string $themePath): void
    {
        ...
        $script = view('arixlog::scripts.update', [
            'panel' => base_path(),
            'runDir' => $runDir,
            'backupDir' => $backupDir,
            'version' => $version,
            ...
            'registerTool' => base_path('app/Extensions/ArixLog/tools/register-provider.php'),
            'versionTool' => base_path('app/Extensions/ArixLog/tools/sync-version.php'),
    ...
    public function prepareRollback(UpdateRun $run): string
    {
        ...
        $script = view('arixlog::scripts.rollback', [
            'panel' => base_path(),
