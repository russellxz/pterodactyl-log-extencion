...
class InstallGuard
{
    ...
    public function stop(Server $server, string $mode, string $by, bool $notifyOwner = true): array
    {
        ...
        //    mata de verdad un contenedor de instalacion atascado.
        if ($mode === self::MODE_FORCE_ROTATE) {
            try {
                $this->daemon->setServer($server)->delete();
                $wingsDeleted = true;
            } catch (\Throwable $e) {
                $warnings[] = 'No se pudo borrar el servidor en el nodo: ' . $this->short($e);
            }
        }
    ...
    private function closeInstallEvent(Server $server, string $by, int $minutes, ?array $rotation, string $mode, bool $wingsDeleted): void
    {
        ...
        $event->fill([
            ...
            'notes' => $wingsDeleted
                ? 'Servidor borrado en el nodo para cortar el contenedor de instalacion. Hay que recrearlo antes de reinstalar.'
                : 'La instalacion se marco como fallida en el panel.',
        ])->save();
    }
    ...
    private function notifyOwner(Server $server, int $minutes, ?array $rotation, array $warnings): void
    {
        ...
        try {
            Mail::send('arixlog::mail.install-stopped', [
                'user' => $user,
                'server' => $server,
                'minutes' => $minutes,
                'rotation' => $rotation,
