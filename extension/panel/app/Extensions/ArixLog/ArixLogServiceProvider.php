...
namespace Pterodactyl\Extensions\ArixLog;
...
use Pterodactyl\Extensions\ArixLog\Http\Middleware\InjectAssets;
use Pterodactyl\Extensions\ArixLog\Listeners\MailLogListener;
use Pterodactyl\Extensions\ArixLog\Listeners\ServerInstallListener;
use Pterodactyl\Extensions\ArixLog\Support\Settings;

/**
 * Punto de entrada de la extension ArixLog.
 *
 ...
 *    de app/Extensions/ArixLog y de public/extensions/arixlog.
 ...
 */
class ArixLogServiceProvider extends ServiceProvider
{
    ...
    {
        $this->loadViewsFrom(__DIR__ . '/resources/views', 'arixlog');
    ...
    /**
     * @arixicon('nombre', 16) dibuja un icono SVG en linea.
     *
    ...
    {
        \Illuminate\Support\Facades\Blade::directive('arixicon', function ($expression) {
            return "<?php echo \\Pterodactyl\\Extensions\\ArixLog\\Support\\Icons::svg({$expression}); ?>";
        });
    }
    ...
    private function registerSchedule(): void
    {
        ...
        $this->app->booted(function () {
            try {
                ...
                $schedule->command('arixlog:watch-installs')
                    ->everyMinute()
                    ->withoutOverlapping(5)
                    ->runInBackground();

                $schedule->command('arixlog:sample')
                    ->everyMinute()
                    ->withoutOverlapping(5)
                    ->runInBackground();

                $schedule->command('arixlog:prune')
                    ->dailyAt('04:10')
                    ->withoutOverlapping();
            } catch (\Throwable $e) {
                $this->reportSilently($e);
    ...
    {
        try {
            logger()->warning('[ArixLog] fallo al arrancar la extension: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
            ]);
        } catch (\Throwable) {
