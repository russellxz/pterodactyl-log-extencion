...
{
    protected $signature = 'arixlog:panel-update
    ...
    public function handle(PanelUpdater $updater): int
    {
        ...
        if ($code !== 0) {
            ...
            $this->line('Para deshacerla: php artisan arixlog:panel-rollback ' . $run->id);

            return self::FAILURE;
        }

        $this->info('Panel actualizado.');
        $this->line('Si algo va mal: php artisan arixlog:panel-rollback ' . $run->id);
