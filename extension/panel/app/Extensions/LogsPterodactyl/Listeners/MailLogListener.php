 ...
 * marca como fallidas el comando logspterodactyl:prune.
 */
class MailLogListener
{
    ...
    /**
     * Campana en curso, si el correo sale de un envio en bloque.
     *
     * Se marca aqui y no buscando la fila despues de enviar: un UPDATE con
     * ORDER BY y LIMIT no funciona igual en todos los motores de base de datos
     * y ademas podria etiquetar la fila equivocada si dos correos van al mismo
     * destinatario a la vez.
     */
    private static ?int $campaign = null;

    public static function withCampaign(?int $id): void
    {
        self::$campaign = $id;
    }
    ...
    private function guard(callable $callback): void
    {
        try {
            ...
            try {
                logger()->debug('[LogsPterodactyl] no se pudo registrar un correo: ' . $e->getMessage());
            } catch (\Throwable) {
                // Nada mas que hacer.
            }
        }
