...
class MailsController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('logspterodactyl::admin.mails', [
            'mails' => $mails,
            'status' => $status,
            'search' => $search,
            'summary' => [
    ...
    {
        $log = MailLog::query()->findOrFail((int) $mail);

        return view('logspterodactyl::admin.mail-detail', ['mail' => $log]);
    }
     ...
     */
    public function preview(string $mail): Response
    {
        $log = MailLog::query()->findOrFail((int) $mail);
     ...
     */
    public function resend(Request $request, string $mail)
    {
        $log = MailLog::query()->findOrFail((int) $mail);
        ...
        if ($to === []) {
            return back()->with('logspterodactyl_error', 'No hay ninguna direccion de correo valida a la que enviar.');
        }

        if (($log->body_html === null || $log->body_html === '') && ($log->body_text === null || $log->body_text === '')) {
            return back()->with('logspterodactyl_error', 'Este correo se guardo sin cuerpo, no se puede reenviar. Activa "guardar el cuerpo" en la configuracion para que los proximos si se puedan.');
        }
        ...
        try {
            ...
            return back()->with('logspterodactyl_error', 'No se pudo reenviar: ' . $e->getMessage());
        }
        ...
        return back()->with('logspterodactyl_success', 'Correo reenviado a ' . implode(', ', $to) . '.');
    }
    ...
    public function test(Request $request)
    {
        ...
        if (!filter_var($to, FILTER_VALIDATE_EMAIL)) {
            return back()->with('logspterodactyl_error', 'Escribe una direccion de correo valida.');
        }

        try {
            Mail::send('logspterodactyl::mail.test', [
            ...
            return back()->with('logspterodactyl_error', 'No se pudo enviar: ' . $e->getMessage());
        }

        return back()->with('logspterodactyl_success', 'Correo de prueba enviado a ' . $to . '.');
    }
}
