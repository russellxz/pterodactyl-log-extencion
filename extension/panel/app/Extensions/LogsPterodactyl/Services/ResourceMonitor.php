...
class ResourceMonitor
{
    ...
    {
        $key = 'logspterodactyl:node-usage:' . $node->id;
        ...
        $this->cachePut($key, $usage);

        return $usage;
    }

    /**
     * La cache es un apano para no machacar a los nodos con una peticion por
     * cada recarga de la pantalla, no algo imprescindible. Si el almacen de
     * cache no esta disponible (el caso tipico: storage/framework/cache sin
     * permisos para www-data) se sigue adelante sin cache en lugar de tumbar
     * la peticion entera, que es lo que pasaba antes.
     */
    ...
    {
        try {
            Cache::put($key, $value, self::CACHE_SECONDS);
        } catch (\Throwable $e) {
            $this->warnOnce($e);
        }
    }
    ...
    /**
     * Un panel con la cache rota generaria una linea de log por cada peticion
     * y llenaria el archivo. Se avisa una vez cada diez minutos y basta.
     */
    private function warnOnce(\Throwable $e): void
    {
        ...
        try {
            logger()->warning(
                '[LogsPterodactyl] no se pudo usar la cache del panel, se sigue sin ella. '
                . 'Suele ser por permisos en storage/framework/cache. Detalle: '
