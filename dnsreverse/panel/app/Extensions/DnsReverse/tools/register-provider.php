...
/**
 * Registra (o quita) el proveedor de DNS Reverse en config/app.php.
 *
 * Es un script suelto, sin nada de Laravel, porque tiene que poder ejecutarse
 * justo despues de una actualizacion del panel o del tema, momento en el que
 * el proveedor todavia no esta registrado y por tanto los comandos artisan de
 * la extension no existen.
 *
...
const PROVIDER = 'Pterodactyl\\Extensions\\DnsReverse\\DnsReverseServiceProvider::class';
const MARKER_START = '// DnsReverse START (gestionado por la extension, no editar a mano)';
const MARKER_END = '// DnsReverse END';
...
$registered = str_contains($contents, 'DnsReverseServiceProvider');
...
if ($mode === 'remove') {
    ...
    foreach ($lines as $line) {
        ...
        if (str_contains($line, 'DnsReverseServiceProvider')) {
            continue;
        }
    ...
    echo "Proveedor de DnsReverse quitado de config/app.php\n";
    exit(0);
}
...
echo "Proveedor de DnsReverse registrado en config/app.php\n";
exit(0);

/**
 * Posicion del corchete que cierra el array 'providers', contando la
 * anidacion para no confundirse con los arrays de dentro.
 */
...
function write(string $path, string $contents, string $panel): void
{
    ...
    $temporary = tempnam(sys_get_temp_dir(), 'dnsreverse');
    ...
    if (!is_file($path . '.dnsreverse-backup')) {
        @copy($path, $path . '.dnsreverse-backup');
    }
